#!/bin/bash
#SBATCH --time=5:00:00 #e.g. 24:00:00, 1-13:00:00
#SBATCH --job-name=cutesv_force
#SBATCH --mem=36G #e.g. 100G
...
#SBATCH --array=1-8,10-12 #e.g.--array=0,6,16-32, --array=0-15:4 (equivalent to --array=0,4,8,12), --array=0-15%4 (limit number of simultaneously running jobs) 
...
apptainer exec -e -B /home -B /scratch -B /project ~/cutesv_2.0.2.sif \
    cuteSV --max_cluster_bias_INS 100 --diff_ratio_merging_INS 0.3 --max_cluster_bias_DEL 200 --diff_ratio_merging_DEL 0.5 \
        ...
        cutesv2.force.${sample_name}.vcf \
        ./ \
        -Ivcf ../12bufo.cutesv.vcf
